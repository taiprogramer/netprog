...
class Server {
    private static final int ECHO_PORT = 3000; // to avoid root permission required on Linux
	...
	Thread t = new Thread(() -> {
	    ...
	    System.out.println(String.format("Addr: %s:%d | Connected", ip, port));
	    try {
		...
		System.out.println(String.format("Addr: %s:%d | Disconnected", ip, port));
	    }
	});
	t.start();
    }
